...
| 단계 | Phase 1 | Phase 2 | Phase 3 (현재) |
|------|---------|---------|----------------|
| 문서 처리 | 페이지 단위 | 섹션 단위 | 섹션 단위 + 구조 보존 |
| 검색 | 키워드 (BM25 유사) | 키워드 (섹션 레벨) | 하이브리드 (BM25 + FAISS) + 리랭킹 |
| 생성 | Ollama (llama3.2) | Ollama (llama3.2) | Ollama (gemma3:4b), temperature=0 |
| 대화 | 싱글턴 | 싱글턴 | 멀티턴 (세션 기반, 쿼리 재작성) |
| 참고 링크 | 페이지 단위 | 섹션 직접 이동 | 섹션 직접 이동 |
