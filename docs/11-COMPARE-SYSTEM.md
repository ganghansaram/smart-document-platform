...
DOCX/PDF 문서 비교 → 듀얼 diff + AI 의미 분류 + 규칙 검증 + 검토 리포트
...
Compare는 **비교·분석·검토 판정 도구**다. 두 문서 버전 간 차이를 시각화하고, AI가 변경의 의미를 자동 분류하며, 검토자가 수락/거절을 판정하여 리포트를 생성한다.

Translator와 달리 **서버에 상태를 저장하지 않는 휘발성 설계**:
- 비교 결과, 판정, AI 분류는 모두 브라우저 메모리에만 존재
- 새로고침하면 초기화 — 결과 보존이 필요하면 리포트를 내보내기
- 서버는 텍스트 추출, 규칙 검증, AI 분류만 담당하는 무상태(stateless) 백엔드
...
| 검증 모드 | 프리셋/커스텀 규칙 엔진, 6종 내장 규칙 |
| 검토 리포트 | 수락/거절 판정 + AI 분류를 텍스트 리포트로 내보내기 |
...
| 권한 | 비교/AI 분류: viewer 이상 / 규칙 저장: admin |
...
- **검증 모드**: 단일 문서의 규칙 위반 검사, 이슈 하이라이트
...
백엔드
├── backend/api/compare.py         ← Compare API 라우터
├── backend/services/compare_service.py ← 텍스트 추출, 규칙 엔진, AI 분류
└── backend/config.py              ← COMPARE_AI_* 설정
