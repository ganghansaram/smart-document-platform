...
캡션 판정은 산출물이 다른 두 계층으로 나뉩니다.

| 계층 | 판정 | 산출 | 기준 |
|------|------|------|------|
| **참조 캡션** | `converter.py` `_detect_caption()` | `class` + `id` + 본문 참조 링크 | 번호 뒤 구분자 필수 (엄격) |
| **표시 캡션** | `converter.py` `_is_display_caption()` | `class` 만 | 구분자 불필요 (느슨) + 조사 배제 + 150자 길이 가드 |

표시 캡션의 본문 오탐 방어선은 2겹입니다.

1. **조사 배제** — 번호에 조사가 바로 붙으면 캡션이 아니라 서술이다.
   `표 1을 보면` · `그림 3과 같이` · `Table 9를 참조한다` → 본문.
   번호 뒤가 공백이면 캡션 (`표 1 주요 제원`, `표 3.1 제원`).
2. **길이 가드** — 조사 없이 길게 이어지는 설명 문장을 거른다 (150자).

판정을 나눈 이유는 **오탐 비용이 다르기** 때문입니다. `id` 는 중복 시
`semantic_checks` 의 `caption_id_unique` 를 위반하고 본문에 잘못된 참조 링크를
만들지만, `class` 의 오탐은 가운데 정렬·마진에 그칩니다. 표기 편차를 흡수하되
피해를 표시 계층에 가두는 구조입니다.

표시 캡션 기준은 프론트 폴백(`js/app.js` `optimizeContent()` 2단계)과 **의도적으로
동일**합니다. Explorer 는 `엔진 ∪ JS` 합집합으로 동작하지만 standalone(exe) 출력은
엔진 단독이므로, 한쪽만 바꾸면 두 화면이 어긋납니다. 동치는
`tests/test_caption_tiers.py` 의 `check_js_parity()` 가 고정합니다.

> JS 폴백은 제거하지 않습니다 — 이미 `contents/` 에 등록된 HTML 은 재변환 전까지
> 옛 출력이므로, 폴백을 빼면 기존 문서의 캡션 표시가 깨집니다.
