...
플랫폼은 **세 가지 운영 환경**을 공식 지원합니다. 본인 환경에 해당하는 열만 확인하세요.

| 구분 | A. 개발 PC (집) | B. 회사 Linux VM | C. 회사 Windows PC |
|------|-----------------|------------------|--------------------|
| **용도** | 개발·테스트 | **주 서비스** | 대안 서비스 |
...
## 5. 환경 C — 회사 Windows PC (톰캣 + Python 직접 실행)

Docker 가 없는 회사 Windows PC 에서 운영하는 **대안 방식**입니다. 프로젝트 디렉토리를 통째로 복사한 뒤 두 개의 프로세스(Tomcat + Python)를 직접 구동합니다.
