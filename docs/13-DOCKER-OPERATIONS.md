...
코드만 변경된 경우(Dockerfile이나 패키지 변경 없음) 전체 이미지 대신 패치 파일만 전송합니다.
패치 파일은 수 MB 수준으로, USB나 네트워크로 빠르게 전달할 수 있습니다.

**개발 PC에서 할 일:**

개발 도구(Claude Code)에서 `/docker-release --patch` 실행 → `patch-vX.X.X.tar.gz` 생성
...
|------|--------|------|
| 1 | `cd /opt/smart-document-platform` | 배포 디렉토리로 이동 |
| 2 | `./patch-apply.sh patch-vX.X.X.tar.gz` | 패치 적용 (자동 재시작) |
| 3 | 브라우저에서 정상 동작 확인 | 기존 데이터가 그대로 유지되어야 함 |

> **최초 1회**: `patch-apply.sh` 스크립트를 서버에 복사해야 합니다.
> **주의**: Dockerfile, requirements.txt, docker-compose.yml이 변경된 경우에는 패치가 아닌 전체 이미지 업데이트(10-1, 10-2)를 사용합니다.

### 10-4. 업데이트 유형 선택 기준

| 변경 내용 | 업데이트 방법 | 전송 크기 |
|-----------|-------------|----------|
| Python 코드, JS/HTML/CSS, 설정 파일 | **패치** (`patch-apply.sh`) | 수 MB |
| Dockerfile, requirements.txt 변경 | **전체 이미지** (`deploy.sh`) | 수 GB |
| docker-compose.yml 변경 | **전체 이미지** + yml 교체 | 수 GB |
| 새 패키지(pip/apt) 추가 | **전체 이미지** | 수 GB |
