    (function() {
        ...
        function applyViewMode(mode) {
            ...
            if (mode === 'list') {
                $docGrid.classList.add('doc-list-mode');
                $btnViewList.classList.add('active');
                $btnViewTile.classList.remove('active');
            } else {
                $docGrid.classList.remove('doc-list-mode');
        ...
        function createDocCard(doc) {
            ...
            var webTranslated = doc.web_translated_pages || 0;
            var total = doc.total_pages || doc.pages || 0;
            if (doc.has_legacy_translation) {
                statusEl.classList.add('status-done');
                statusEl.textContent = '번역완료 (전체)';
            } else if (translated > 0 || webTranslated > 0) {
                statusEl.classList.add('status-partial');
                var parts = [];
                if (translated > 0) parts.push('PDF ' + translated + '/' + total);
                if (webTranslated > 0) parts.push('웹뷰 ' + webTranslated + '/' + total);
        ...
        function updateRightPanel() {
            ...
            if (translateEngine === 'web') {
                ...
                if (wStatus === 'done') {
                    showRightWebView();
                    updateToolbarForStatus('done');
                } else if (wStatus === 'translating') {
                    showRightTranslating(wps);
                    ...
                    fetch(API + '/api/translator/web-translate/' + currentDocId + '/page/' + currentPage + '/status', {
                        ...
                        if (st.status === 'done' || st.status === 'translating' || st.status === 'error') {
                            webPageStatusCache[String(currentPage)] = st;
                        }
                        if (st.status === 'done') {
                            showRightWebView();
                            updateToolbarForStatus('done');
        ...
        function stopWebPolling() {
            if (webPollingTimer) {
                clearInterval(webPollingTimer);
                webPollingTimer = null;
            }
        }
        ...
        function _applyWebFontSize() {
            if ($webViewContent) $webViewContent.style.fontSize = webFontSize + 'px';
        }
        ...
        $translateBtn.addEventListener('click', function() {
            ...
            if (translateEngine === 'web') {
                // 웹 뷰 번역 모드
                fetch(API + '/api/translator/web-translate/' + currentDocId + '/page/' + currentPage, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ model: model }),
                    credentials: 'include',
                    ...
                    webPageStatusCache[String(currentPage)] = {
                        status: 'translating',
                        progress_stage: '추출 준비 중...',
                        model: model,
                    };
                    updateRightPanel();
                }).catch(function(err) {
        ...
        function renderSearchResults(data, query) {
            ...
            if (memos.length > 0) {
                ...
                memos.forEach(function(m) {
                    html += '<button class="ts-search-item" data-action="open" data-doc="' + escAttr(m.doc_id) + '" data-page="' + m.page + '" data-source="memo">';
        ...
        // 검색 결과 소스에 따라 적절한 패널 자동 열기
        function _openPanelForSearchSource(source) {
            if (source === 'translated') {
                if (activeRailPanel === 'web-translate') return;
                var webBtn = $iconRail.querySelector('[data-panel="web-translate"]');
                if (webBtn && !webBtn.disabled) webBtn.click();
            } else if (source === 'memo') {
                if (activeRailPanel === 'memo') return;
                var memoBtn = $iconRail.querySelector('[data-panel="memo"]');
                if (memoBtn && !memoBtn.disabled) memoBtn.click();
            }
        }
        ...
        // 현재 페이지 MD 다운로드
        $dlWebPage.addEventListener('click', function() {
            _closeDownloadMenu();
            if (!currentDocId || $dlWebPage.disabled) return;
            _downloadFile(
                API + '/api/translator/web-view/' + currentDocId + '/page/' + currentPage,
                'translated_p' + currentPage + '.md',
                function(r) { return r.json().then(function(d) { return new Blob([d.markdown], {type: 'text/markdown'}); }); }
            );
        });

        // 전체 문서 MD 다운로드
        $dlWebFull.addEventListener('click', function() {
            _closeDownloadMenu();
            if (!currentDocId) return;
            _downloadFile(
        ...
        function _downloadFile(url, filename, blobFn) {
            fetch(url, { credentials: 'include' })
                .then(function(r) {
                    if (!r.ok) throw new Error('다운로드 실패');
                    return blobFn ? blobFn(r) : r.blob();
                })
                .then(function(blob) {
