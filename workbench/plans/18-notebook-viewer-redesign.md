...
- ✅ 패널 리사이즈 핸들 (드래그 25~75%, components.css 재사용, 하이라이트 제거)
- ✅ 용어집 입력란 헤더 통합 + 카운트 표시
- ✅ 메모 패널 빈 상태 안내 (이미 Phase 4에서 구현됨)
- ✅ 폰트 토큰 통일 (cancel-btn, page-info, zoom-level → 토큰)
- ✅ 다크모드 전체 검증 (4개 패널 + 싱글 뷰, range-dialog 토큰화)
