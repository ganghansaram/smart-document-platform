...
- ✅ PDF 번역 패널 내부 UI (모델 선택, 번역/취소, 범위 번역, 상태 텍스트) — DOM appendChild로 이동
- ✅ 번역 PDF.js 렌더링 — side-panel 내부에서 정상 렌더
- ✅ 스크롤 동기화 — sp-controls 내부로 이동 (PDF 전용 표시)
- ✅ 폴링 로직 — 기존 동작 유지 (패널 열림 상태에서 정상)
- ✅ 웹뷰 컨트롤도 동시 이관 (폰트 크기, 전체 문서 토글)
- ✅ 닫기/전환 시 컨트롤 툴바 복원 (_restoreControlsToToolbar)

### Phase 3: 웹뷰 번역 패널 이관 — 1.5일

- ✅ 웹뷰 패널 내부 UI (모델 선택, 번역/취소, 폰트 크기, 전체 문서 토글) — Phase 2에서 동시 이관
- ✅ Markdown 렌더링 — side-panel 내부에서 정상 렌더
- ✅ 전체 문서 연속 스크롤 + IntersectionObserver — 기존 코드 그대로 동작
- ✅ 폴링 로직 — 기존 동작 유지
- ✅ 인라인 번역 버튼 — 기존 showRightPending 로직 유지
...
| 2 | PDF 번역 패널 이관 | 1.5일 | ✅ |
| 3 | 웹뷰 번역 패널 이관 | 1.5일 | ✅ (Phase 2에서 동시 완료) |
| 4 | 메모 패널 + 용어집 패널 | 1.5일 | ⬜ |
...
**Phase 1a~5 합계**: ~8일 (Notebook 레이아웃 재설계 핵심)
