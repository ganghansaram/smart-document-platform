...
### 3.3 패널 열림 상태 — 확장 모드 (예: 마인드맵)

원문 PDF를 완전히 밀어내고 전체 영역을 사용.
NotebookLM의 마인드맵 뷰와 동일한 패턴.
...
| 순서 | 아이콘 | 패널 이름 | 패널 모드 | 내용 |
|:----:|:------:|----------|:--------:|------|
| 1 | 📄 | PDF 번역 | 일반 (50%) | 모델 선택, 번역/취소, 범위 번역, 스크롤 동기화, 번역 PDF 뷰어 |
| 2 | 📝 | 웹 뷰 번역 | 일반 (50%) | 모델 선택, 번역/취소, 폰트 크기, 전체 문서 토글, Markdown 렌더 |
| 3 | 📋 | 메모 | 일반 (50%) | 메모 목록 (페이지별), 메모 검색, 메모 추가 |
| 4 | 📖 | 용어집 | 일반 (50%) | 용어 목록, 추가/편집/삭제 (현재 모달 → 패널로 이관) |
| — | — | 구분선 | — | 상단: 문서 도구 / 하단: AI·분석 |
| 5 | 🤖 | AI 요약·Q&A | 일반 (50%) | 문서 요약 + 챗봇 Q&A (탭 구성) |
| 6 | 📊 | 문서 요약 | 일반 (50%) | 핵심 문장, 키워드, 통계 (향후, disabled) |
| 7 | 🧠 | 마인드맵 | **확장 (전체)** | 문서 구조 시각화 — 헤딩/키워드 기반 트리 (향후, disabled) |
...
4. **두 가지 패널 모드**:
   ...
   - **확장 모드** (100%): 원문 PDF를 완전히 밀어냄. 마인드맵 등 넓은 공간이 필요한 도구용. 패널 상단에 "원문으로 돌아가기" 버튼.
...
<div class="viewer-panels" id="viewer-panels">
    ...
    <div class="icon-rail" id="icon-rail">
        ...
        <button class="rail-btn" data-panel="mindmap" data-panel-mode="expand" title="마인드맵" disabled>
            <svg>...</svg>
        </button>
    </div>
</div>
