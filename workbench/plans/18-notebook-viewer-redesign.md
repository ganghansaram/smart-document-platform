...
- ⬜ **마킹 플로팅 위젯(`marking-float`) → 메모 패널로 대체**
  - 기존 `marking-float` 위젯 제거 (플로팅 아이콘 + 드롭다운)
  - 동일한 데이터·API (`get_annotations`, `create_annotation` 등) 재사용
  - 마킹 색상(4색 하이라이트), 페이지 라벨, 메모 텍스트 등 시각적 표현 그대로 상속
  - 사이드 패널에서 전체 문서 메모 목록 (페이지별 그룹)
  - 메모 항목 클릭 → 좌측 PDF 해당 페이지·위치로 스크롤
  - 마킹 생성은 기존과 동일 (좌측 PDF 텍스트 선택 → 팝오버)
