...
| Phase | 제목 | 상태 | 커밋 | 비고 |
|-------|------|------|------|------|
| 계획서 | 작성 + 결정 시트 + 현업 피드백 반영 | ✅ 완료 (2026-04-30) | — | 본 문서 |
| **Phase 0** | 회귀 방어망 강화 (테스트 스크립트·골든셋) | ✅ 완료 (2026-04-30) | (예정) | sim_label_consistency C2/T1 패턴 + 골든셋 5건 신규 |
| **🔴 Phase 1** | 백엔드 점수 공식 v3 통일 (`× 0.5` 제거) — 자동 파급 (verdict·tiers·이력 등) | ✅ 완료 (2026-04-30) | (예정) | similarity_engine.py 1라인 수정, 골든셋 5/5 PASS |
| **🔴 Phase 2** | 배너·보고서 baseline 정합 + 단일 점수 표시 | ✅ 완료 (2026-04-30) | (예정) | 화면 "원점수" 제거, 보고서 score_baseline 신규 |
| **Phase 3** | 옛 3단계 임계값 잔존 정리 (이력 색상·verdict_legacy) | ✅ 완료 (2026-04-30) | (예정) | 이력 색상 v3 verdict 기반, 25/74 임계 정합 |
| **Phase 4** | 분모 계산 정합 (sentence index set) | ⏸ 대기 | — | 🟡 위험 중 — 별도 단계 |
| **Phase 5** | 사이드바·그룹 헤더 단위 표기 통일 | ⏸ 대기 | — | Phase 4 직후 |
| **Phase 6** | 약한 유사 ⓧ 버튼 제거 (점수 상승 역설 해소) | ⏸ 대기 | — | 독립 적용 가능 |
| **Phase 7** | 마감 (NaN 방어 + 단위 표기 + 디버그 로그) | ⏸ 대기 | — | 마지막 |

**상태 범례**: ⏸ 대기 / 🟡 진행 중 / ✅ 완료 / ⏹ 보류
**우선순위 묶음**:
- 🔴 **Critical 묶음** = Phase 0+1+2+3 (현업 부서 피드백 직결, 다음 릴리즈 v2.8 핵심 안건)
- 🟡 **완전 묶음** = Phase 0~7 전체 (분모·단위·역설·방어 마감)
