...
| 65 | 홈 하단 "목록/이력" 영역 표현 정합 | ⬜ 착수 대기 (시안 탐색 선행) | 3시스템 하단이 제각각(Verify=틀 없는 맨 리스트 / Notebook·Author=카드). Verify 최근작업 틀 정합 + 섹션 헤더 어휘 통일. **표현만, 기능(저장/재실행) 무관.** 배지·점수색 불가침. 시안 미리보기 선행 |
