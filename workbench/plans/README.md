...
| 66 | Notebook 트리 패널 도킹(밀어내기) 모드 | 🟢 착수 가능 (결정 확정 · 구현 미착수) | **② 도킹-듀얼(두 문서+트리 동시) + 리사이즈 완충.** 오버레이 디폴트·도킹은 새 토글 `tp-docked`(핀 보존, B안)·전역 상태·min-width 180/320(시작값). design-reviewer 검증(340px·`rerenderBothPanels` 재사용·A2). 잔여=트리 리사이즈 핸들 구현. 시안 `reports/tree-panel-dock-variants-mockup.html` |
