...
| # | 제목 | 상태 | 비고 |
|---|------|------|------|
| 72 | Author 문서 워크스페이스 (셸 통합+소유자 한정+좌측 패널+Explorer 분리) | draft (방향 캡처) | 5부분: 편집기 오버레이→**Author 셸 마운트(공통 헤더 유지)**·저작면 현실화(골격 프리셋 제거)·Explorer 분리·소유자만 노출·Notebook식 좌측 패널. Plan-71 시각 자산 이식(오버레이 껍데기만 대체). 저작 문서 0개=최저 비용 |
