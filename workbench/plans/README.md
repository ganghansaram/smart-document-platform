...
| 67 | Explorer 트리·문서 생애주기 관리 (삭제 cascade·인덱스 정합·detach·재시작 audit) | ⬜ 착수 가능 (v1.1, plan-advisor 통과) | 사전조사+검토 완료(공수 6.5일). 벡터 `IndexFlatL2.remove_ids` 증분삭제(타입변경·재임베딩 불필요), 삭제정책=하드삭제+cascade+휴지통 폴더 이동. authored 삭제=Plan-67 소유(개명/큐레이션만 60). 동시성 락·중복 url 노드 반영. 멀티워커 캐시 비해당(단일워커) |
