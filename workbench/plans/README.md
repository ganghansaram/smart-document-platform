...
| 62 | 공통 진입 카드(`.entry-card`) 추출 + Verify 허브 카드 현대화 | ⬜ 착수 대기 (계획 승인 단계) | Author 진입 타일이 Verify 허브 카드보다 세련됨(평상시 elevation 차이) → 공통 `.entry-card`를 components.css에 추출, Verify는 스킨만 채택(모드 3색·가운데정렬 유지), Author는 베이스 전환(시각 무변). 복붙 아닌 추출(드리프트 방지) |
