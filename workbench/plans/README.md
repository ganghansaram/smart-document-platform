...
| 68 | Explorer 안정화·성능복원·관리자 올클린 | ⬜ draft (협의 대기) | 사용자 식별 이상 6+요구1. 진단 3건 완료(원인 코드근거 확보). **Phase 0 진단 게이트** 후 회귀복구(업로드500)·성능(인덱싱 관측/폴백)·정합(OS직접삭제 고아)·올클린 초기화·메뉴 토스트. [plans/68-*.md](68-explorer-stabilization-perf-adminreset.md) |
