...
## ⚠️ L2(Gateway) 활성화 선행조건 — 필수 (개발 책임자 결정, 2026-06-13)

> L2 를 켤 때는 아래 3가지를 **한 묶음으로 함께 테스트**한다. 단독으로 하나만 먼저 하지 않는다
> — 검증 불가 코드가 핵심 경로에 방치되는 것을 막기 위함 (progressive delivery: "쓸 시점에, 검증 가능한 상태로").

**L2 활성화 = 다음 3단계를 한 번에:**

1. **채팅(RAG) 경로 게이트웨이 이식** — 현재 게이트웨이는 요약·Q&A·비교만 경유하고
   **Explorer RAG 채팅(최다 트래픽 경로)은 우회 중.** 토글을 켜기 전 반드시 이식해야
   "단일 진입점" 명분이 온전해진다.
   - 대상 5~6곳: `llm_client.py:125/128/156` (⚠️ **sync 경로 1곳** — 계획 본문이 다른 sync 경로를 미룬 것과 동일한 함정), `rag_agent.py:133`, `question_router.py:42`, `query_decomposer.py:92`
   - 미이식 시 토글 ON 해도 채팅은 `OLLAMA_MAX_QUEUE` 서버 큐에만 의존
2. **부하 테스트 (Phase 2c)** — 개발 PC 스모크 + 회사 VM 실로드로
   `LLM_GATEWAY_MAX_CONCURRENT`/`STREAM_SLOTS`/`MAX_QUEUE` 실측값 확정
3. **토글 ON** (관리자 설정 → 공통 탭 → "AI 동시성 제어") + 위 1·2 포함 회귀 테스트

### 착수 트리거 — 지켜볼 단일 지표
- 관리자 대시보드 **"피크 동시 AI 요청 수"** (= 접속자 수가 아니라 *같은 순간 실제 LLM 호출 수*)
- **4~5 도달·지속 시 → 위 L2 묶음 착수.**

### 임계값 현실 보정 (업계 데이터 반영, 2026-06-13)
- 본 계획 본문의 *"Ollama + L2 튜닝으로 동시 10~15명 감당"* 은 **낙관적 추정**.
- 업계 실측 정설: **Ollama 의 쾌적 구간은 동시 추론 4~5개**. 그 이상은 P95 지연이 수초→1분으로
  급락 → 튜닝이 아니라 **엔진 교체(L3/vLLM) 영역**.
- 따라서 L1·L2 는 "수용 천장을 올리는" 수단이 아니라 **공정성·체감품질·관측** 수단으로 본다.
  실제 동시 LLM 사용이 5~6 을 지속 초과하면 L3(vLLM) 검토를 시작한다.
- 근거: vLLM 은 continuous batching 으로 동일 GPU 에서 수십 동접 처리, Ollama 는 동시 4~5
  초과 시 큐잉으로 지연 붕괴 (업계 다수 벤치 일치).
...
- **L3 (vLLM) 은 여전히 보류**. Ollama + L2 튜닝으로 100명/동시 10~15명은 감당 가능. ⚠️ *(2026-06-13 보정: 이 "10~15명" 은 낙관적. 상단 "임계값 현실 보정" 참조 — 쾌적 구간은 동시 4~5개)*
