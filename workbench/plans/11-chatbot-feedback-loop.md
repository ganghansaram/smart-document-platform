...
1. `git pull` 후 백엔드 재시작 (`cd backend && python main.py`)
2. 브라우저 **Ctrl+Shift+R** (강력 새로고침) — JS/CSS 캐시 초기화 필수
3. admin 계정으로 로그인
...
| # | 테스트 | 확인 포인트 |
|---|--------|------------|
| 1 | 챗봇에서 문서 관련 질문 (예: "KF-21 비행시험 요약") | 답변 하단에 👍/👎 아이콘 버튼 표시 |
| 2 | 👍 클릭 | 아이콘이 초록색으로 변경 |
| 3 | 👍 다시 클릭 | 선택 해제 (원래 회색으로 복귀) |
| 4 | 👎 클릭 | 아이콘이 빨간색으로 변경, 이전 👍은 해제 |
| 5 | "안녕하세요" 같은 일반 대화 | 피드백 버튼 미표시 (CHAT 라우팅) |
| 6 | 질문 전송 직후 | 타이핑 인디케이터(점 3개 애니메이션)가 즉시 표시되고, 첫 응답 토큰이 올 때까지 유지 |

#### B. 시스템 프롬프트 설정

| # | 테스트 | 확인 포인트 |
|---|--------|------------|
| 7 | 관리자 설정 → Explorer → AI/RAG 탭 스크롤 | "챗봇 프롬프트" 섹션 존재 |
| 8 | 프롬프트를 "답변은 반드시 3줄 이내로 하세요"로 변경 → 저장 | 재시작 불필요 표시 |
| 9 | 챗봇에서 질문 | 답변이 짧아지는지 확인 (프롬프트 반영) |
| 10 | 프롬프트를 비워두고 저장 → 챗봇 질문 | 기본 프롬프트로 정상 동작 (폴백) |

#### C. 대시보드 피드백 통계

| # | 테스트 | 확인 포인트 |
|---|--------|------------|
| 11 | 관리자 설정 → 대시보드 탭 → 최하단 스크롤 | "챗봇 피드백 분석" 섹션 표시 |
| 12 | 피드백 데이터 없을 때 | "아직 피드백이 없습니다" 메시지 |
| 13 | 위 테스트 A에서 피드백 몇 건 남긴 후 대시보드 새로고침 | 만족도 %, 긍정/부정 건수, 라우트별 테이블, 일별 차트 표시 |
| 14 | 부정 피드백 남긴 후 대시보드 확인 | "최근 부정 피드백" 목록에 해당 질문/답변 표시 |
| 15 | 다크모드 전환 | 모든 UI가 정상 표시 |

### 7-3. 테스트용 질문 예시와 주목 포인트
...
"MIL-STD-810 환경시험 종류"
```
- 주목: 답변 하단 참고 문서 링크 + 피드백 버튼 표시
- 기대: 직접 매칭되는 문서가 있으므로 빠르고 정확한 답변

#### 비교 분석 (COMPARE 라우팅)
```
"KF-21과 F-35의 스텔스 설계 차이점은?"
"구조시험과 환경시험의 차이를 설명해줘"
"MIL-STD-810과 MIL-STD-461 적용 범위 비교"
```
- 주목: 쿼리가 분해되어 여러 문서를 검색한 후 통합 답변
- 기대: 단일 검색보다 풍부한 참고 문서, 비교 형식의 답변

#### 추론 필요 (REASON 라우팅 — Agentic RAG)
```
"KF-21 개발 일정이 지연된 원인을 분석해줘"
"체계개발에서 가장 기술적 난제는 무엇이었나?"
"비행시험 결과가 양산 결정에 미치는 영향은?"
```
- 주목: 여러 번 검색을 반복하며 정보를 축적한 후 종합 답변
- 기대: 검색 1회로는 찾기 어려운 심층 답변. 다만 응답 시간이 더 걸림

#### 일반 대화 (CHAT 라우팅 — 검색 없음)
```
"안녕하세요"
"고마워"
"오늘 날씨 어때?"
```
- 주목: 피드백 버튼이 **표시되지 않아야** 함
- 기대: 문서 검색 없이 즉시 응답

### 7-4. 피드백 루프 활용 시나리오

```
1. 사용자들이 챗봇 사용 → 자연스럽게 👍/👎 남김
2. 관리자가 대시보드에서 확인:
   - 전체 만족도 73%... REASON 경로만 0%?
   - 부정 피드백 목록 → "FY14~20 예산 추세 분석" 질문에서 답변 부실
3. 관리자 판단:
   - "예산 관련 문서가 인덱스에 없구나" → 문서 추가
   - "REASON 라우팅이 너무 느린데?" → 관리자 설정에서 조정
   - "답변 형식이 마음에 안 들어" → 시스템 프롬프트 수정
4. 수정 후 만족도 변화 추적
```

### 7-5. 알려진 제한사항

- 피드백은 **로그인한 사용자만** 가능 (미인증 시 API 401)
- 피드백 데이터는 `data/analytics.db`의 `chat_feedback` 테이블에 저장
- 대시보드 통계는 최근 30일 기준, 일별 차트는 14일
- 브라우저 캐시로 인해 변경 후 **Ctrl+Shift+R** 필요할 수 있음
