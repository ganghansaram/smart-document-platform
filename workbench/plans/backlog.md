...
## Verify — 프리셋+개별 토글 규칙 설정 UI

> 출처: Plan-23 Phase 5i (#4, #5, #9)

- **프리셋 선택 + 개별 규칙 ON/OFF** — ⚙ 설정 모달 확장. 현재 6종만 토글 가능 → 21종 전체 + 소스별 그룹핑
  - 프리셋: MIL-STD-461 검증용 / 기술교범 작성용 / 일반 기술문서 / 커스텀
  - 961 규칙이 현재 기본 OFF인데 활성화할 UI 수단 없음 → 이 작업으로 해결
- **문장길이 중복 해소** — 프리셋에서 custom-sentence-length(문자) / ste-3.1+3.2(단어) 택일 설계
- **억제(suppress) 기능** — 개별 이슈를 "의도적"으로 무시하는 기능. 실사용 피드백 후 판단
