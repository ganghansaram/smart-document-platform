...
Phase 1에서 스킬 2종(`/review-ui`, `/check-theme`)을 함께 만듭니다.
...
> **산출물**: `memory/theme-guide.md`, `.claude/skills/review-ui/SKILL.md`, `.claude/skills/check-theme/SKILL.md`
> **영향 파일**: 없음 (문서 작성만)
...
### Phase 2: CSS 토큰 통합 (main.css :root)

> **목표**: main.css `:root`에 통합 디자인 토큰을 선언하고, 다른 CSS 파일의 중복 변수를 제거한다.
> **영향 파일**: `css/main.css`, `css/admin-settings.css`, `css/analytics.css`
> **위험도**: 중간 (기존 변수 참조가 깨질 수 있음 → 단계적 교체)

#### 현재 문제

```
main.css         → --active-color: #0066cc    (12개 변수)
admin-settings   → --as-primary: #0066cc      (23개 변수, 독립)
analytics        → --ad-primary: #0066cc      (12개 변수, 독립)
translator.html  → #0066cc 하드코딩 26회      (변수 없음)
```
...
1) main.css :root 확장
   theme-guide.md에 정의된 CSS 변수를 main.css의 :root(라이트)와
   body[data-theme="dark"](다크)에 선언해줘.

   기존 12개 변수(--primary-navy, --active-color 등)는 유지하되,
   새 변수를 추가하고 기존 변수에서 새 변수를 참조하도록 매핑해.
   예: --active-color: var(--color-primary);

   추가할 변수 카테고리:
   - 색상: --color-primary, --color-success, --color-warning, --color-danger, --color-info
   - 배경: --bg-page, --bg-card, --bg-input, --bg-hover
   - 텍스트: --text-primary, --text-secondary, --text-muted
   - 테두리: --border-default, --border-light
   - 포커스 링: --focus-ring
   - 스페이싱: --space-xs ~ --space-2xl
   - 라운딩: --radius-sm ~ --radius-full
   - 그림자: --shadow-sm, --shadow-md, --shadow-lg, --shadow-focus
   - 트랜지션: --transition-fast, --transition-normal, --transition-slow

2) admin-settings.css 변수 교체
   --as-primary → var(--color-primary), --as-danger → var(--color-danger) 등으로 교체.
   admin-settings 고유 변수(--as-tab-bg 등 레이아웃 전용)는 유지하되,
   공통 색상은 :root 변수를 참조하도록 변경.
   다크모드 블록에서 중복 색상 정의 제거.

3) analytics.css 변수 교체
   --ad-primary → var(--color-primary) 등으로 교체.
   analytics 고유 변수(--ad-bar-color 등 차트 전용)는 유지.
   다크모드 블록에서 중복 색상 정의 제거.

4) 각 파일 변경 후 기존 CSS 셀렉터에서 변수 참조가 깨지지 않는지 확인.
   교체된 변수 매핑 테이블을 보여줘.
...
[3-1] 공유 컴포넌트 기본 클래스를 main.css에 추가
   theme-guide.md의 컴포넌트 규격을 기반으로 main.css 하단에 추가:
