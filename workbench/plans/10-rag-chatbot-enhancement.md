...
- [ ] 기존 Tier 1 질문 5개 회귀 테스트 (답변 품질 비교)
- [ ] 스트리밍 응답 지연 시간 측정 (첫 토큰 < 2초 목표)
- [ ] 프로바이더 추상화 단위 테스트

### Phase 2 검증
- [ ] 비교 질문 테스트: "A와 B의 차이점" → 양쪽 문서 모두 검색되는지
- [ ] 멀티홉 질문 테스트: "A에서 참조하는 B의 내용" → 2단계 검색 동작
- [ ] 메타데이터 필터: 카테고리별 검색 정확도
- [ ] 컨텍스트 확장 후 VRAM 안정성 (장시간 운용 테스트)

### Phase 3 검증
- [ ] 질문 라우터 정확도: 20개 테스트 질문 분류 정확도 80% 이상
