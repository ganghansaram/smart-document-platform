...
| # | 질문 | 기대 답변 핵심 | 검증 포인트 |
|:-:|------|--------------|------------|
| T1-1 | "감항인증이란?" | 용어 정의 | 용어집/문서에서 정확히 추출 |
| T1-2 | "복합재 수리 절차를 알려줘" | 절차 순서 나열 | 순서 정확성, 누락 여부 |
| T1-3 | "MIL-STD-810의 진동 시험 조건은?" | 수치/스펙 | 정확한 수치 인용 |
| T1-4 | "형상관리 절차를 설명해줘" | 절차 개요 | 관련 섹션 검색 적중 |
| T1-5 | "엔진 시험의 주요 항목은?" | 항목 나열 | 출처 문서 정확성 |

#### Tier 2 — 비교/종합 (5문항)

| # | 질문 | 기대 답변 핵심 | 검증 포인트 |
|:-:|------|--------------|------------|
| T2-1 | "구조 시험과 환경 시험의 차이점은?" | 양쪽 비교 | 두 문서 모두 검색되는지 |
| T2-2 | "비행 시험 전체 절차를 요약해줘" | 3~5문장 요약 | 핵심 누락 없이 종합 |
| T2-3 | "동체 설계와 주익 설계의 공통 기준은?" | 교차 참조 | 멀티 문서 종합 능력 |
| T2-4 | "시스템 요구사항에서 참조하는 규격은?" | 멀티홉 | 참조 추적 (A→B) |
| T2-5 | "품질관리와 형상관리의 관계를 설명해줘" | 관계 설명 | 두 개념 연결 |

#### Tier 3 — 추론/분석 (5문항)

| # | 질문 | 기대 답변 핵심 | 검증 포인트 |
|:-:|------|--------------|------------|
| T3-1 | "이 시험 절차에서 빠진 항목이 있을까?" | 갭 분석 | 추론 깊이 |
| T3-2 | "구조 시험 결과가 설계 기준을 만족하는지 판단해줘" | 근거 기반 판단 | 논리적 추론 |
| T3-3 | "레이더 시스템과 통신 시스템의 간섭 가능성은?" | 분석적 답변 | 크로스 도메인 추론 |
| T3-4 | "운용 매뉴얼에 정비 절차가 충분히 반영되어 있어?" | 교차 검증 | 문서 간 정합성 판단 |
| T3-5 | "초도 비행 전 필수 점검 항목을 정리해줘" | 종합 정리 | 다중 출처 종합 + 구조화 |
...
#### Phase 1 비교: gemma3:4b → gemma3:27b

| # | 질문 | 모델 | 응답시간 | 검색적중 | 정확도 | 완성도 | 환각 | 비고 |
|:-:|------|:---:|:---:|:---:|:---:|:---:|:---:|------|
| T1-1 | 감항인증이란? | 4b | _초 | O/X | _/5 | _/5 | O/X | |
| | | 27b | _초 | O/X | _/5 | _/5 | O/X | |
| T1-2 | 복합재 수리 절차 | 4b | _초 | O/X | _/5 | _/5 | O/X | |
| | | 27b | _초 | O/X | _/5 | _/5 | O/X | |
| ... | ... | ... | ... | ... | ... | ... | ... | |
...
|:-----:|:---:|:---:|:---:|:---:|:---:|
| Before | gemma3:4b | _/5 | _/5 | _/5 | _초 |
| Phase 1 | gemma3:27b + 스트리밍 | _/5 | _/5 | _/5 | _초 |
...
5. 결과 파일 저장: `workbench/test-results/rag-baseline-4b.md`

> **중요**: 베이스라인 없이 고도화를 시작하면 개선 효과를 정량적으로 입증할 수 없음.
...
### Phase 1 검증
- [ ] **베이스라인 측정 완료** (8.5절, 4b 모델로 15문항 기록)
