...
> 작성일: 2026-03-19
> 최종 갱신: 2026-03-22
> 상태: Phase 1~3+ 완료 / Phase 4a~5 미착수
> 브랜치: `plan17-library`
...
### Phase 4a: 웹 뷰 읽기 경험 + 즉시 가치 UX — 2~3일
...
**별도 리팩토링 (기능 안정화 후):**
- translator.js 전략 객체 패턴 분리
- 번역 속도 최적화 (블록 병렬 호출)
- page-header/footer 텍스트 제거 옵션

### Phase 5: UI 리네이밍 + 텍스트 모드 정리 — 1일
...
| Phase | 내용 | 예상 공수 | 상태 |
|:-----:|------|:--------:|:----:|
| 1 | 추출 파이프라인 (PyMuPDF4LLM + 사전 검증) | 2~3일 | ✅ |
| 2 | 번역 파이프라인 (블록 번역 + 표 셀 + 일괄 최적화) | 3~4일 | ✅ |
| 3 | 프론트엔드 (웹 뷰 + API + Markdown 렌더링) | 5~6일 | ✅ |
| 3+ | 사용자 피드백 + 품질 개선 (이미지/표/수식/스타일) | — | ✅ |
| 4a | **웹 뷰 읽기 경험** (전체 연속 스크롤 + 싱글/듀얼 + 다운로드) | 2~3일 | ⬜ |
| 4b | 검색 + 다운로드 확장 (원문/번역 구분 + ZIP) | 1일 | ⬜ |
| 4c | 편집기 (EasyMDE + 모달 + 저장/재병합) | 1.5~2일 | ⬜ |
| 4d | PDF 탐색 개선 (썸네일 사이드바) + 고급 기능 | 2일 | ⬜ |
| 5 | UI 리네이밍 + 텍스트 모드 정리 (엔진 2종 축소) | 1일 | ⬜ |

**Phase 1~5 합계**: ~19.5일 — 여기까지 완료하면 **"연속 읽기 + 편집 + 검색이 되는 개인 지식 저장소"** (엔진 2종: PDF | 웹 뷰)
