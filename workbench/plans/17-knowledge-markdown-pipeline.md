...
> 작성일: 2026-03-19
> 최종 갱신: 2026-03-22
> 상태: Phase 1~4a 완료 / Phase 5→4b 순서로 진행 예정
> 브랜치: `plan17-library`
...
> **핵심 목표**: 웹 뷰를 "주 읽기 경험"으로 격상. Library 비전의 첫 단추.
...
**배경**: 현재 웹 뷰는 페이지 단위로 Markdown을 표시한다. 하지만 Library의 핵심 가치는
...
> **주의**: 기존 `text_translated.pdf`가 있는 문서의 하위 호환 확인 필요.
> `meta.json`에 `text_translate` 상태가 남아있는 문서 → 데이터는 보존, UI에서 노출만 제거.
> `textPageStatusCache`, `startTextPolling()` 등 텍스트 전용 코드 정리 대상 파악 필요.

### Phase 4b: 검색 + 다운로드 확장 — 1일

> Phase 5에서 엔진 정리 후 착수. 백엔드 `translated_pages` + `match_source` 이미 구현 완료,
> 프론트엔드 검색 UI 확장이 주요 작업.

- ⬜ 검색 결과 원문/번역 구분 UI 표시
- ⬜ ZIP 다운로드 (MD + 이미지 assets 포함)

### Phase 4c: 편집기 — 보류 (실사용 피드백 후)

> 실사용 피드백 수집 후 착수 권장. 편집 필요성과 수준을 확인한 뒤.
> EasyMDE ~400KB + CodeMirror CSS 충돌 검증 필요 — 수요 확인 전 번들 추가는 비효율.

- ⬜ EasyMDE 번들 (`js/lib/easymde/`) + 다크모드 CSS 검증
- ⬜ 편집 모달 (glossary 패턴: `.modal-overlay` + `.modal-box` + 3종 닫기)
- ⬜ 편집 저장 API + marked.js 즉시 재렌더링 + `full_translated.md` 재병합

### Phase 4d: PDF 탐색 개선 + 고급 기능 — 보류 (실사용 피드백 후)

> 핵심 워크플로우에 필수가 아님. Library 실사용 경험 축적 후 우선순위 재평가.

- ⬜ **PDF 썸네일 사이드바** (페이지 전체 구조 한눈에 파악 + 빠른 이동)
  - 좌측 패널 내 토글 가능한 썸네일 스트립 (PDF.js `getPage().render(scale~0.15)`)
  - 현재 페이지 하이라이트, 클릭 시 `goToPage(N)` 호출
  - 번역 완료 페이지에 시각적 표시 (체크 뱃지 또는 테두리 색상)
...
| **6** | **5** | **UI 리네이밍 + 텍스트 모드 정리 (엔진 2종 축소)** | **1일** | **⬜ 다음** |
| **7** | **4b** | **검색 구분 UI + ZIP 다운로드** | **1일** | **⬜** |
| — | 4c | 편집기 (EasyMDE + 모달 + 저장/재병합) | 1.5~2일 | 보류 |
| — | 4d | PDF 탐색 개선 (썸네일 사이드바) + 고급 기능 | 2일 | 보류 |

**핵심 완결 (Phase 5 + 4b)**: ~2일 — 텍스트 모드 정리 + 리네이밍 + 검색 확장으로 **Library 핵심 워크플로우 완결**
