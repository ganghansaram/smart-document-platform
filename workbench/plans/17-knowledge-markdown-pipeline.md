...
### Phase 3+: 사용자 피드백 + 품질 개선 — 진행 중

**완료:**
- ✅ 엔진 토글(PDF|텍스트|웹뷰) 툴바 가장 우측 끝으로 이동 (위치 흔들림 해소)
- ✅ Markdown 스타일 Explorer 패턴 통일 (max-width 900px, line-height 1.8, 좌측 정렬)
- ✅ 이미지 추출: 임베디드 추출 → 영역 캡처(get_pixmap) 전환 (잘림 방지, 업계 표준)
- ✅ 이미지+캡션: `<figure>` + `<figcaption>` 래핑 (HTML5 표준, 가운데 정렬)
- ✅ 이미지 크기: 페이지 대비 폭 비율(%) 적용 (Explorer converter.py 패턴)
- ✅ figure 태그 전후 빈 줄 보장 (제목 ## 렌더링 수정)
- ✅ done 상태 툴바: 웹뷰 캐시에서 모델/시간 표시 ([웹뷰] 접두사)

**진행 중:**
- ✅ 제목 이탤릭(`_..._`) 제거 — 제목 가독성 개선
- ⬜ `table_mode="image"` 시 표 영역 이미지 캡처 구현 — picture 캡처와 동일 방식 (MinerU 패턴)
- ⬜ 제목 레벨 개선: PDF TOC(목차 트리) 활용 → 다양한 문서 테스트 후 적용

**알려진 한계 (현재 허용):**
- 벡터 그래픽(drawings) 이미지 약간 잘림 — PyMuPDF4LLM bbox 감지 정밀도 한계, 패딩 3px 적용
- 번역 속도가 PDF 번역보다 느림 — Ollama 블록별 호출 (향후 병렬화 검토)
- 복잡한 병합 표 (colspan/rowspan) 추출 깨짐 — `table_mode="image"`로 전환하여 대응
...
### Phase 4: 편집 + 다운로드 + UX 개선 — 3일

- ⬜ EasyMDE 번들 + 다크모드 CSS 검증
...
- ⬜ Markdown/전체 병합/이미지 포함 ZIP 다운로드
- ⬜ 싱글/듀얼 적응형 레이아웃
...
| 3+ | 사용자 피드백 + 품질 개선 (이미지/스타일/제목) | 진행 중 | 🔄 |
