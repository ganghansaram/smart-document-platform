...
### 🔴 착수 전 반드시 결론낼 2건 (P4 설계 좌우)
1. **정적서빙 누수 대책** — authored `.md` 직접 fetch(`app.js:431`·`editor.js:80`)를 어떻게 막나: (a) 인증 백엔드 엔드포인트 경유(`GET /api/authored/{id}` 게이팅) vs (b) 정적 루트 밖 이전. 이걸 안 정하면 소유자 한정이 성립 안 함.
2. **소유권 저장 위치** — 서버측 owner_id(sidecar JSON·인덱스) vs front matter. 검토 결론 = **서버측 권장**(front matter는 편집가능·id 아님).

### 협의 (설계 시)
3. **셸 통합 방식** — (a) 뷰 전환 vs (b) 좌측 패널 고정+우측 편집 스플릿. **검토 권장 = (b) 스플릿**(Author는 단일 본문·PDF 듀얼페인 없음 → VS Code/Acrobat식 자연스러움, Plan-66 도킹 정합). 셸 소유 = `author.html`, 편집기는 우측 마운트. 주의: topbar close 버튼이 셸 nav와 중복 가능.
4. **저작면 기본 구조** — 완전 빈 문서 vs 최소 현실 골격(제목만/첫 문단). 통일 양식(표지) 절충.
5. **admin 가시성** — admin은 전 계정 저작 문서 열람? (큐레이션 목적)
6. **좌측 패널 재사용 깊이** — Plan-66 패널은 import 불가(translator 결합) → **UI 패턴 신규 구현** 전제. 어디까지 Notebook 룩앤필을 맞출지.
