...
- ⬜ **4-1. 원본 PDF 다운로드**
  - 기존 `GET /api/translator/pdf/{doc_id}` 에 `Content-Disposition: attachment` 추가
  - 또는 프론트엔드에서 blob URL + `<a download>` 방식

- ⬜ **4-2. 현재 페이지 번역 PDF 다운로드**
  - 기존 서빙 API (`/translated-pdf/{doc_id}/page/{page_num}`, `/text-translated-pdf/{doc_id}/page/{page_num}`) 활용
  - 프론트엔드에서 현재 보고 있는 번역 PDF를 다운로드

- ⬜ **4-3. 프론트엔드 UI**
  - 뷰어 툴바에 다운로드 아이콘 버튼 + 드롭다운 메뉴
  - 옵션: 원본 PDF / 현재 페이지 번역 PDF
  - 번역 미완료 시 "현재 페이지 번역 PDF" 비활성화

- ⬜ **4-4. 검증**
  - 다운로드한 PDF가 정상 열리는지 확인
  - 번역 미완료 페이지에서 다운로드 버튼 상태 확인
  - 파일명에 문서명 + 페이지 번호 포함 확인
...
| 4th | PDF 다운로드 | Phase 4 | 0.5일 | DRM 전 결과물 확보 (축소: 텍스트 내보내기/병합은 Phase 5 이후) |
