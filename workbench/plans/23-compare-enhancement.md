...
> **작성일**: 2026-04-01
> **최종 갱신**: 2026-04-04 (Phase 5 계획 추가)
> **상태**: Phase 3 완료, Phase 3V 대기
...
| 3V | 실효성 검증 (3모드 신뢰도) | 6 | 0 | Phase 3h 후 착수 |
| 4 | 세션 관리 + 이력 | 8 | 0 | 대기 |
...
- 종합 진단 보고서: 모드별 신뢰도 현황 + 측정 근거 + 개선/제거 판단
- 개선된 점수 공식 (코드 반영 완료 상태)
- 테스트 문서셋 (이후 Phase 5 회귀 테스트에 재사용)
- Phase 5 우선순위 권고 (어떤 표준 규칙을 먼저 구현할지)
...
| 5i | 인텔리전스 패널 연동 | 규칙 출처 표시, 카테고리 확장, 필터 추가 | 중 | 5h | ⬜ |
| 5j | 검증 + 캘리브레이션 | 테스트셋 회귀, 오탐률 측정, 임계값 튜닝 | 중 | 5i | ⬜ |

### 5.5 Phase 순서 관계

```
Phase 3 (완료) → Phase 3V (현 시스템 진단)
                      ↓
                 진단 보고서 → Phase 5 우선순위 결정
                      ↓                ↓
              Phase 4 (세션 관리)    Phase 5 (표준 규칙)
              ← 독립 진행 가능 →    ← 3V 결과에 의존 →
```

> Phase 4와 5는 **병렬 가능**하나, Phase 5의 규칙이 Phase 4에서 저장하는 결과 품질에 영향을 주므로
> 실무적으로는 **5 Step 3(STE 규칙)까지 완료 후 4 착수**를 권장.
