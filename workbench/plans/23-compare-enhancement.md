...
| 5 | 표준 기반 규칙 엔진 (STE + MIL-STD) | 11 | 7 | 진행 중 (5a~5g 완료) |
...
| 5h | 스코어링 확장 | confidence 가중치 통합 (writing 카테고리는 5d에서 추가 완료) | 하 | 5f, 5g | ⬜ |
| 5i | 규칙 설정 UI + 패널 연동 | 아래 5i 상세 참조 | 상 | 5h | ⬜ |
...
| # | 항목 | 설명 | 현재 상태 |
|---|------|------|----------|
| 1 | `RULE_LABELS` 동적 생성 | 기존 6종 하드코딩 → JSON 규칙에서 `name_ko` 자동 수집 | 체크리스트에 새 규칙 미표시 |
| 2 | 규칙 출처 배지 | 사이드바 이슈에 `source` 표시 (ASD-STE100, MIL-STD 등) | 이슈 메시지만 표시 |
| 3 | 신뢰도 표시 | 규칙별 `confidence` 배지 (high/medium/low) | 미표시 |
| 4 | 프리셋+개별 토글 UI | ⚙ 설정 모달에서 프리셋 선택 + 개별 규칙 ON/OFF | 기존 6종만 토글 가능 |
