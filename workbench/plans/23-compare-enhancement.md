...
| 3 | 신뢰도 표시 | confidence 필드는 이슈에 포함, UI 배지는 보류 (low 규칙이 기본 OFF라 실익 낮음) | 보류 |
| 4 | 프리셋+개별 토글 UI | 설정 모달 확장 필요 — 별도 태스크로 분리 권장 | 보류 |
| 5 | 억제(suppress) 기능 | 별도 태스크로 분리 권장 | 보류 |
| 6 | `RULE_CATEGORY` 동적 생성 | API에서 category 자동 매핑 | ✅ 완료 |
| 7 | 카테고리별 점수 `writing` | 5d에서 완료 | ✅ 완료 |
| 8 | 내보내기 규칙명 | RULE_LABELS 동적화로 자동 해결 | ✅ 완료 |
| 9 | 문장길이 중복 감지 해소 | 프리셋 UI(#4) 구현 시 택일 설계 | 보류 |
