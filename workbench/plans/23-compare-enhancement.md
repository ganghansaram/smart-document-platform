...
| 5e | 기존 6종 마이그레이션 | JSON 정의 완료 + legacy 위임 작동. 엔진 매처 전환은 보류 | 중 | 5d | ✅ |
...
5d 점검에서 식별된 잔여항목을 포함한 5i 태스크 범위:

| # | 항목 | 설명 | 현재 상태 |
|---|------|------|----------|
| 1 | `RULE_LABELS` 동적 생성 | API `/rule-definitions`에서 name_ko 자동 수집 | ✅ 완료 |
| 2 | 규칙 출처 배지 | 사이드바 이슈에 STE/38784/961 배지 표시 | ✅ 완료 |
| 3 | 신뢰도 표시 | 스코어링에 반영 완료, UI 배지는 과잉 → 불필요 판단 | ~~삭제~~ |
| 4 | 프리셋+개별 토글 UI | 961 규칙 활성화 수단 필요 → **backlog 이관** | → backlog |
| 5 | 억제(suppress) 기능 | 실사용 피드백 후 판단 → **backlog 이관** | → backlog |
| 6 | `RULE_CATEGORY` 동적 생성 | API에서 category 자동 매핑 | ✅ 완료 |
| 7 | 카테고리별 점수 `writing` | 5d에서 완료 | ✅ 완료 |
| 8 | 내보내기 규칙명 | RULE_LABELS 동적화로 자동 해결 | ✅ 완료 |
| 9 | 문장길이 중복 감지 해소 | #4 프리셋 UI에서 custom/STE 택일로 해결 → **#4와 묶어 backlog** | → backlog |
| 10 | 체크리스트 동적 표시 | 활성 규칙 15종 전체 + 출처 태그 | ✅ 완료 |
