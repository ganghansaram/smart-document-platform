# Plan-48 — Verify 유사도 미니맵 정상화 + 표준 기능 보강 ✅ 완료

> Plan-45 (유사도 분류 통합) 완료 후속. 사용자 지적 — "색깔 막대는 보이는데 클릭이 안 되고, 본문 하이라이트와 위치도 어긋나는 것 같다." (2026-04-25 ~ 2026-04-26)

> **종료 — Phase 1+2+Hotfix×3 완료 (2026-04-26).** Phase 3 (코드 통합) 은 ROI 낮아 미진행 결정. 사용자 시각 검증 완료.
...
| **Hotfix** — 위치 정확도 (긴 문서 실측) | ✅ **완료 (2026-04-25)** | `02a4df5` | scroll container 정정 + visible element 필터. 마커 92% 누적 → 분포 정상 |
| **Hotfix2** — 색상 본문/미니맵 일치 | ✅ **완료 (2026-04-25)** | `8522065` | 의역 보라/약한유사 파랑 본문 디자인을 미니맵에 반영. `--sim-*-border` 토큰 통일 |
| **Hotfix3** — 본문 outlier 정정 (전 UI 색상 통일) | ✅ **완료 (2026-04-26)** | `8469c96` | tokens.css 5 토큰 정렬 + 인쇄 리포트 1라인. 의역 파랑/약한유사 회색/자동제외 옅은회색 — 본문·미니맵·사이드바·필터 모두 일치 |
| Phase 3 — diff·sim 코드 통합 | ❌ 미진행 (확정) | — | ROI 낮음, user-facing 변화 0. 다음 미니맵 변경 시 합쳐 처리 권장 |
