...
> 작성일: 2026-03-12
> 최종 갱신: 2026-03-14
> 상태: Phase 5 진행 중 (5-1 CSV 임포트 + 5-2 JSON 내보내기/가져오기)
...
- [ ] **5-1. CSV 일괄 업로드 (용어 사전 임포트)**
  - [ ] 금지 용어 CSV 업로드 (`term,replacement` 형식)
    - 기존 규칙 모달 금지 용어 편집 패널 하단에 "CSV 가져오기" 버튼 추가
    - `<input type="file" accept=".csv">` → FileReader → 파싱
  - [ ] 일관성 그룹 CSV 업로드 (`group_name,term1,term2,...` 형식)
    - 동일 패턴: 일관성 그룹 편집 패널 하단에 "CSV 가져오기" 버튼 추가
  ...
  - [ ] **기본 샘플 데이터 제공** — 기술 문서 작성 가이드(KS, ISO, 공공기관 기준) 기반
    - 금지 용어 예시: 구어체→문어체 ("할 수 있다"→"가능하다", "이것"→"해당 항목" 등)
    - 일관성 그룹 예시: 외래어 통일 ("서버/Server/써버"), 약어 통일 ("SW/소프트웨어")
    - 용도: 규칙 시스템 미경험 사용자가 참고·커스터마이징할 출발점
    - 규칙 모달에 "샘플 데이터 불러오기" 링크 제공

- [ ] **5-2. 규칙 세트 내보내기/가져오기 (JSON)**
  - [ ] "내보내기" → 현재 프리셋을 JSON 파일 다운로드
    - 규칙 모달 하단 또는 헤더에 "내보내기" 버튼
    - `Blob` + `<a download>` 패턴 (Phase 4 txt 내보내기와 동일)
  - [ ] "가져오기" → JSON 파일 업로드 → 프리셋 추가/덮어쓰기
    - `<input type="file" accept=".json">` → 스키마 검증 → 적용
