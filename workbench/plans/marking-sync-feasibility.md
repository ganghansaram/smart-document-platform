...
### Phase 2: 형광펜 마킹 + 좌우 동기화

텍스트 선택 → 하이라이트 생성/저장/복원 + 우측 동기화 표시. 이 단계 완료 시: 기본 형광펜 동작 + 양쪽 표시.
...
5. 우측 동기화 표시
   - 좌측에서 생성된 하이라이트를 우측 annotation-layer에도 동일 % 좌표로 표시
   - 생성/삭제/페이지 이동 시 양쪽 동시 반영
   - `renderAnnotationsRight()` — 우측 전용 렌더 함수
...
| 2 | 형광펜 마킹 + 좌우 동기화 | 텍스트 선택 → 하이라이트 생성/복원/양쪽 표시 | Phase 1 |
