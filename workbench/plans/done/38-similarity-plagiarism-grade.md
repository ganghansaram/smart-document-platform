...
> **개정 2**: 2026-04-22 사용자 승인 + Claude 판단 반영 — 5건 수정
> **개정 3**: 2026-04-23 R-feedback 즉시 권고 2건 적용 (사이드바 라벨 2x2 그리드, 온보딩 트리거 이전)
> **상태**: ✅ **완료** (2026-04-23) — 11단계 중 9단계 적용 완료. 잔여 2건은 시기/타 Plan 의존 사유 보류
> **종합 검증**: `workbench/reports/38-feedback-report.md` 참조 — 골드셋 14/14 PASS, 콘솔 에러 0, K-SPEC 실무 투입 준비 완료
...
| 2 | 2 | Phase 5 — 모달 A + 온보딩 (R2) | ✅ | 2026-04-23 | 2축 다이어그램 + 6라벨 표 + 1회 자동 워크스루 |
| 2 | 3 | 검사 설정 UI 토글 + 모달 C | ✅ | 2026-04-23 | 5체크박스 즉시 재계산 + localStorage + 모달 C 가이드 |
| 2 | 4 | L3 가이드 페이지 (R4-B) | ✅ | 2026-04-23 | verify-guide.html 유사도 챕터 7섹션 (면책/분류/산식/설정/알고리즘/보고서/FAQ) |
| 2 | 4 | 사이드바 3단 (R4-A, 선택) | ⏸️ | — | 1:1 시나리오에서 효용 낮음. 1:N 확장 시 함께 |
| 2 | 4 | 재캘리브레이션 (R4-C) | ⏸️ | — | 실 사용 1~2주 후 사례 흡수 |

**범례**: ✅ 완료 / ⏳ 진행중 또는 다음 / ⏸️ 대기

**상태**: ✅ **Plan-38 완료** (2026-04-23). 11단계 중 9단계 ✅ + R-feedback 2건 ✅. 잔여 2건은 시기/타 Plan 의존 (사이드바 3단 — 1:N 확장 시 / 재캘리브레이션 — 실 사용 1~2주 후).
...
- ✅ **묶음 2 단계 4 — L3 가이드 페이지 (R4-B) 완료** (2026-04-23)
  - `contents/guide/verify-guide.html` 유사도 검사 챕터 7섹션 신규
    - 면책 (callout, "유사도 ≠ 표절", Crossref 가이드 인용)
    - 분류 체계 (4그룹 표 + 6세부 표, 어휘×의미 2축 설명)
    - 점수 산출법 (산식 pre-block + 5단계 신호등 표)
    - 검사 설정 옵션 (5옵션 표 + 자동 제외 2건)
    - 도구·알고리즘 근거 (Schleimer 2003, BAAI 2024, Turnitin/iThenticate/카피킬러 인용)
    - 보고서 출력 활용법 (HTML/Excel/TXT 형식 표 + 출처 마커 설명)
    - FAQ 5건 (낮은 점수/번역 라벨/재검사/거의동일 vs 의역/인쇄 색상)
  - GUIDE-STYLE.md 준수 (po-* 클래스, 1컬럼, 이모지 X, 담백한 톤)
  - 검증: Playwright fetch + DOMParser, 7/7 섹션 + 9 표 + 3 callout + formula pre 모두 정상, 가이드 메뉴 진입 시각 확인
- ⏸️ 사이드바 3단 (R4-A) — 1:N 확장 시 함께 (1:1에서 효용 낮음)
- ⏸️ 재캘리브레이션 (R4-C) — 실 사용 1~2주 후 사례 흡수
