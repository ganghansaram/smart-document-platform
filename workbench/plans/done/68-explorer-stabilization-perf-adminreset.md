...
> 상태: ✅ 완료 (2026-07-05, 개발 축) — 코어(P0 진단·P1·P2·P4 올클린·P3 D4 빈폴더·P5 F1·F2·F4 토스트) **코드완료 + 로컬검증**. 완료 정의=코드완료+로컬검증(회사 배포는 완료 조건 아님, `CLAUDE.md`). 잔여 이관: **회사 VM 배포·F3(세션만료 재현) → `workbench/DEPLOY-QUEUE.md`** · **D2/D3(관측 경량화, 계획서상 "선택")·Phase 6(업계표준 비교 문서, 회사 데이터 필요) → `backlog.md`**.
...
> 읽는 법: **"집 코드"가 ✅면 개발 끝**, **"회사 배포"가 ⏳면 회사 서버 반영만 남음.**
> 범례: ✅ 완료 · 🟡 일부 · ⬜ 시작 전 · ⏳ 배포 대기 · — 해당 없음 · 🏠 지금 집에서 가능 · 🏢 회사 가야 함

| # | 무엇 (쉬운 말) | 집 코드 | 회사 배포 | 한마디 |
|---|----------------|:------:|:--------:|--------|
| 0 | 원인 진단 | ✅ | — | 끝남 |
| 1 | 큰 파일 업로드 지원 | ✅ | ⏳ | 다 짬 · 회사 반영만 |
| 2 | 색인 속도·GPU 상태 표시 | ✅ | ⏳ | 다 짬 · 회사 반영만 |
| 3 | 삭제 후 빈 폴더 정리 | 🟡 | ⏳ | D4(핵심) ✅ · D2·D3 관측은 선택 |
| 4 | 관리자 전체 초기화 | ✅ | ⏳ | 다 짬 · 회사 반영만 |
| 5 | 알림 토스트 + 항목추가 버그 | 🟡 (F3 남음) | ⏳ | F3은 🏢 회사 재현 필요 |
| 6 | 업계표준 비교 문서 | ⬜ | — | 🏠 지금 집에서 가능 |

**한눈 요약:** 집에서 짤 코드는 대부분 끝. 남은 건 ① 회사 서버 배포(1·2·4·5) · ② 집에서 할 2개(3·6) · ③ 회사서 재현할 F3.
(Phase별 상세 상태·근거는 아래 **Tasks** 체크박스와 **Progress Log** 참조. 총 예상 공수 ~6.5일.)

> Phase 우선순위 (⚠️ 설계검토 후 조정) = **2(성능)** > 1(업로드) > 5(UX) > 4(초기화) > 3(정합) > 6(문서). 협의 후 조정.
...
- [x] D4. **빈 부모 폴더 정리 완료** — `_prune_empty_parents(url)` 신설: 부모가 비면 위로 **연쇄 정리**, `contents/` 최상위·시스템 폴더(`_ALLCLEAN_PRESERVE`) 보호, 빈 폴더는 복구데이터 없어 **rmdir 하드삭제**(안 비면 OSError→중단). `delete_document_by_url` 에 배선(`pruned_dirs` 반환). 업계표준=물리 폴더(파생물)는 정리·메뉴 노드(menu.json, 독립 실체)는 유지 → 삭제를 업로드의 대칭으로. 집 검증: 파일시스템 4케이스+통합 전부 PASS.
...
## Progress Log
- 2026-07-05 — **Phase 3 D4 완료(집, 코드+검증).** 삭제 후 빈 물리 폴더 잔존 해소. `_prune_empty_parents(url)` 신설 — 부모 폴더가 비면 위로 연쇄 정리, `contents/` 최상위·시스템 폴더(`_ALLCLEAN_PRESERVE`) 보호, 빈 폴더는 복구데이터 없어 rmdir 하드삭제(안 비면 OSError→중단). `delete_document_by_url` 에 배선(`pruned_dirs` 반환). **업계표준 정합**: 물리 폴더(업로드 시 생기는 파생 저장통)는 비면 정리 = S3 프리픽스·git·`os.removedirs` 방식 / 메뉴 트리 노드(menu.json, 사용자 조직 단위=독립 실체)는 유지 = Confluence·CMS 방식 → 삭제를 업로드의 완벽한 대칭으로. 검증: 파일시스템 4케이스(연쇄·비었을때만·최상위보호·시스템보호)+통합(`delete_document_by_url` 전체경로) 모두 PASS, 테스트 잔재 0, py_compile OK. 남음=D2·D3(관측, 선택)·회사 배포(백엔드 이미지 재빌드).
