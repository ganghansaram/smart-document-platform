...
- ✅ **다크 모드 텍스트 가독성** — `_fixMmTextColor()` + MutationObserver로 foreignObject div 색상 직접 제어
- ✅ **루트 노드 제목** — 첫 번째 헤딩을 루트로 사용 (백엔드 `build_mindmap_tree` 수정)
- ✅ **노드 클릭 → 섹션 이동** — SVG 이벤트 위임, 클릭한 노드 텍스트 → 웹뷰 해당 헤딩 스크롤
- ✅ **키워드 가지 시각 구분** — "키워드" → "Keywords" 라벨로 변경, 섹션과 구분
- ✅ **확장 모드 SVG 재조정** — `_mmInstance.fit()` 호출로 대응
- ✅ **테마 전환 동기화** — `body[data-theme]` MutationObserver로 라이트↔다크 전환 시 텍스트 색상 실시간 보정

#### Step 5b: NotebookLM 수준 인터랙션 강화 — ✅

> 2026-03-30 NotebookLM 벤치마크 조사 기반. 초기 펼침·컨트롤 버튼 추가.

- ✅ **`initialExpandLevel: 1`** — 초기 depth 1만 표시 → 점진적 탐색 경험
...
#### Step 7: 노드 클릭 → Q&A 채팅 연동 (Phase 4와 병행 검토)

> NotebookLM 핵심 차별점. 노드 클릭 시 해당 주제로 Q&A 탭 전환+질문 자동 입력.

- ⬜ 마인드맵 노드 클릭 시 Q&A 탭 활성화 + 해당 노드 텍스트를 질문으로 프리필
- ⬜ "이 섹션에 대해 더 설명해줘" 형태의 프롬프트 자동 생성
- ⬜ 기존 Q&A 인프라 재사용 (notebook_chat.py)
