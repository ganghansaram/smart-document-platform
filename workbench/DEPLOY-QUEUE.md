...
- **대상 버전**: **v2.14** — 패키지 **빌드·검증 완료** (2026-07-29, 기준 커밋 `da26695`, v2.12 컷 이후 37 커밋)
- **반입물 위치**: `release-v2.14/` — 7z 5분할(1GB 단위) + `deploy.sh`·`patch-apply.sh`·`docker-compose.yml`·`.env.example` + `README-반입.md`(반입 절차서). **그대로 복사해 가면 됨**
  - 루트 `platform-v2.14.tar` 는 2026-08-13 폴더 정리 시 삭제(7z 무결성 검증 후). 필요하면 `7z x platform-v2.14.7z.001` 로 복원 — 보관 정책은 `memory/project_deployment_tar_retention.md`
- **배포유형**: **전체 이미지** (Plan-68·72 backend 변경 有 → nginx+backend 재빌드. 이미 빌드본에 반영됨)
- **체크리스트**:
  - [ ] `.env` `OLLAMA_URL=host.docker.internal:11434` 확인
  - [ ] `7z x platform-v2.14.7z.001` → tar 복원 (서버측, `p7zip-full` 필요)
