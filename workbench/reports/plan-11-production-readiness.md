...
| **데이터 보호** | 9/10 | ✅ | ~~백업 스크립트~~ → Plan-22 완료 (원자적 쓰기, 일일 백업) |
| **API 보안** | 7/10 | ✅ | 에러 메시지 정리 (0.5일) |
| **서버 구성** | 5/10 | ⚠️ | 백엔드 서비스화 (0.5일) |
| **로깅/모니터링** | 8/10 | ✅ | ~~로깅 설정~~ → Plan-22 완료 (RotatingFileHandler, 헬스체크) |
| **에러 처리** | 8/10 | ✅ | 폴백 체인 + 그레이스풀 셧다운 + 스턱 태스크 복구 |
| **프론트엔드** | 8/10 | ✅ | Tomcat이 이미 프로덕션급 |
| **DB 구조** | 8/10 | ✅ | SQLite 유지, 백업 완료 |
...
| 5 | **에러 메시지 정리** | ⏳ 미완 | 내부 정보 노출 방지 |
| — | **원자적 JSON 쓰기** | ✅ 완료 | tmp→rename 패턴 (Plan-22) |
| — | **그레이스풀 셧다운** | ✅ 완료 | 태스크 취소, 서브프로세스 정리 (Plan-22) |
| — | **스턱 태스크 복구** | ✅ 완료 | 서버 시작 시 자동 리셋 (Plan-22) |
| — | **헬스체크** | ✅ 완료 | `GET /api/health` (Plan-22) |
