...
DEFAULT_SYSTEM_PROMPT = """당신은 KF-21 전투기 기술 문서 전문 어시스턴트입니다. 제공된 참고 문서만을 기반으로 답변합니다.
...
[언어]
한국어로 답변합니다.
참고 문서가 영어여도 한국어로 번역하여 답변합니다.
영어 기술 용어는 한국어 설명 뒤 괄호 안에 병기합니다 (예: 능동전자주사배열 레이더(AESA))."""


def _get_system_prompt():
    """config.CHAT_SYSTEM_PROMPT가 설정되어 있으면 사용, 아니면 기본 프롬프트 폴백"""
    custom = getattr(config, 'CHAT_SYSTEM_PROMPT', '')
    ...
    try:
        ...
        if loop and loop.is_running():
            ...
            with concurrent.futures.ThreadPoolExecutor() as pool:
                answer = pool.submit(
                    asyncio.run,
                    provider.generate(prompt, system=_get_system_prompt())
                ).result(timeout=120)
        else:
            answer = asyncio.run(provider.generate(prompt, system=_get_system_prompt()))
...
async def generate_response_stream(
    ...
    async def token_stream():
        async for token in provider.generate_stream(prompt, system=_get_system_prompt()):
            yield token
